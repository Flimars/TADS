...
CREATE TABLE residencia (
    ...
    tamanho CHARACTER VARYING(10) CHECK (tamanho IN ('pequena', 'media', 'grande')) NOT NULL,
...
CREATE OR REPLACE PROCEDURE agendar_faxinas(
    diarista INTEGER,
    residencia INTEGER,
    tipo_agendamento CHARACTER VARYING,
...
BEGIN
    RETURN QUERY
    SELECT
        d.id AS d_diarista_id,
        d.cpf,
        d.nome,
        r.id AS r_residencia_id,
        (r.cidade || ', ' || r.bairro || ', ' || r.rua || ', ' || r.numero || 
        COALESCE(' - ' || r.complemento, '')) AS endereco_residencia,
        f.data_faxina,
        f.valor_definido,
        f.valor_pago,
        f.feedback
